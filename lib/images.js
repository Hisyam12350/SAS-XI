...
export function getImagePath(item) {
  ...
  // Prioritas 1: Gunakan gambar yang diupload jika ada
  if (item.gambar) {
    return `/uploads/${item.gambar}`;
  }

  // Prioritas 2: Name-based map untuk matching (untuk data lama)
  const nameMap = {
